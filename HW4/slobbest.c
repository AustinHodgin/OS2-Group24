 ...
 */
 ...
 /*References used : Stackoverflow, how to test slob.c, Zainkai repo,
  								    compilor errors were fixed in order to get this file to
 										compile with this version of the yocto kernel.
  */
 ...
 static void *slob_page_alloc(struct page *sp, size_t size, int align)
 {
	slob_t *prev, *cur, *aligned = NULL;
 	...
 	for (prev = NULL, cur = sp->freelist; ; prev = cur, cur = slob_next(cur)) {
		...
		if((slob_last(cur)||best_fit == 0)&&(best_cur!=NULL)){
...
/*
 * This function checks to see if there is space in the current page. It goes through
 * the page's list of blocks and checks the space available, returning a number.
 * The number can be -1 if there is no space, 0 if it is a perfect fit, or the number of
 * blocks that will be left over if allocation happens. Best fit aims to minimize the return value
 *
*/
...
/*These two functions are used by our system calls to return the
 *free and total space. We call the pointer for a small, medium, or
 *large slob, and iterate through the pages. Total space counts pages
 *and multiplies by page size, free space gets the units value from
 *each page and returns the sum of all of them
 */
asmlinkage long sys_slob_total_space(void){
	...
	printk("slob: Getting total space\n");
  ...
  slob_list = &free_slob_small;
	...
	spin_unlock_irqrestore(&slob_lock, flags);
	space = num_pages * SLOB_UNITS(PAGE_SIZE);
	printk("slob: Total space is %lu\n",space);
	return space;
}
...
asmlinkage long sys_slob_free_space(void){
	...
	printk("slob: Getting free space\n");
